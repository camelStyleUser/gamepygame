...
enemies=[]
resources=[]
inventory={'wood':0,'stone':2,'flammable':0,'metal':2,'food':0}
angle=0
health=10
qprs=False
winter=random.randint(0,1)==0
flame=False
freeze=0
prs=False
run=True
...
while run:
  ...
  if delay>waitdelay:
      lastresspawn=now
      waitdelay*=1.05
      resources.append(Resource(random.randint(0,surface.get_width()),random.randint(0,surface.get_height()),random.choice(spawnin)))
  ...
  if winter:
    label=font.render("cold level:"+str(math.floor(freeze/10))+"."+str(math.floor((freeze/10-math.floor(freeze/10))*100)),1,(255,0,255))
    surface.blit(label,(0,0))
  if pygame.time.get_ticks()<5000:
    label=font.render("do not play if not invited by",1,(255,0,0))
    surface.blit(label,(0,surface.get_height()/2))
    label=font.render("nRzJx5AgBR=(disc us id)",1,(255,0,0))
    surface.blit(label,(0,surface.get_height()/2+100))
  ...
  label1=font.render("flammable:"+str(inventory["flammable"])+" metal:"+str(inventory["metal"]),1,(255,0,255))
